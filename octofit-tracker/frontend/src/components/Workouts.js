...
function Workouts() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.log('Workouts normalized items:', normalizedItems);
  ...
  const filteredItems = items.filter((item) => {
    const haystack = `${item.workout_name} ${item.intensity} ${item.target_minutes} ${item.user}`.toLowerCase();
  ...
  return (
    <section className="card shadow-sm">
      <div className="card-body">
        ...
        <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
          <div className="col-sm-9 col-md-10">
            <input
              ...
              type="search"
              placeholder="Search by workout, intensity or user"
              value={query}
        ...
        {error && <p className="text-danger">Workouts API error: {error}</p>}
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h3 className="h5 modal-title mb-0">Workout Detail</h3>
