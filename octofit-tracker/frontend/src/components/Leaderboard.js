...
function Leaderboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.log('Leaderboard normalized items:', normalizedItems);
  ...
  const filteredItems = items.filter((item) => {
    const haystack = `${item.user} ${item.rank} ${item.points}`.toLowerCase();
  ...
  return (
    <section className="card shadow-sm">
      <div className="card-body">
        ...
        <form className="row g-2 mb-3" onSubmit={(event) => event.preventDefault()}>
          <div className="col-sm-9 col-md-10">
            <input
              ...
              type="search"
              placeholder="Search by rank, user or points"
              value={query}
        ...
        {error && <p className="text-danger">Leaderboard API error: {error}</p>}
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h3 className="h5 modal-title mb-0">Leaderboard Detail</h3>
