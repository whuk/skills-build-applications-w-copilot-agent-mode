...
function Teams() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.log('Teams normalized items:', normalizedItems);
  ...
  const filteredItems = items.filter((item) => {
    const haystack = `${item.name} ${item.universe} ${item.description || ''}`.toLowerCase();
  ...
  return (
    <section className="card shadow-sm">
      <div className="card-body">
        ...
        {error && <p className="text-danger">Teams API error: {error}</p>}
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h3 className="h5 modal-title mb-0">Team Detail</h3>
